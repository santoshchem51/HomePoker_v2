...
describe('TransactionService', () => {
  ...
  describe('recordCashOut', () => {
    ...
    it('should validate minimum cash-out amount', async () => {
      await expect(
        transactionService.recordCashOut('session-1', 'player-1', 0.005) // Below MIN_CASH_OUT
      ).rejects.toThrow(ServiceError);
    });

    it('should validate maximum cash-out amount', async () => {
      await expect(
        transactionService.recordCashOut('session-1', 'player-1', 1000.01) // Above MAX_CASH_OUT
      ).rejects.toThrow(ServiceError);
    });
